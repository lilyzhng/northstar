...
import { FeedbackItem, ActaTaskSettings, ActaFeedbackData, FEEDBACK_TRIGGER_TAGS } from "./types";
...
export class FeedbackManager {
	...
	updateData(data: ActaFeedbackData): void {
		this.data = data;
	}
	 ...
	 */
	hasFeedbackTag(line: string): boolean {
		const tags = this.extractTags(line);
		return FEEDBACK_TRIGGER_TAGS.some((triggerTag) =>
	...
	async removeFeedback(itemId: string): Promise<void> {
		if (!this.data.addedFeedback[itemId]) return;
		delete this.data.addedFeedback[itemId];
		await this.saveData();
		new Notice("Feedback removed from board");
	}
